...
import { Timeline, TimelineItem, TimelineDot, TimelineContent, type DotVariant } from "@/components/ui/timeline";
...
const STATUS_DOT_VARIANT: Record<string, DotVariant> = {
  nouvelle: "blue",
  en_cours: "amber",
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="border-t border-slate-100 pt-4 space-y-3">
            ...
            {comments.map((c) => {
              ...
              const svgUrl = isDelivery ? lines.find((l) => /^https?:\/\/\S+\.svg$/i.test(l.trim())) : undefined;
              return (
...
export function AdminRequestsSection() {
  const { requests, loading, fetchAll, assignToMe, updateStatus } = useAdminRequests();
    ...
    return (
      <RequestDetailPanel
        ...
        onAssign={async (id) => {
          ...
          if (ok) setSelectedRequest((prev) => prev ? { ...prev, status: "en_cours", assignedTo: "moi" } : prev);
          return ok;
        }}
        onUpdateStatus={async (id, status, extra) => {
          const ok = await updateStatus(id, status, extra);
          if (ok) setSelectedRequest((prev) => prev ? { ...prev, status, ...extra } : prev);
          return ok;
        }}
      />
    );
