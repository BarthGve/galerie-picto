...
- **Migrations Drizzle** : Ne jamais creer les fichiers SQL manuellement. Toujours utiliser `pnpm drizzle-kit generate` dans `backend/`. Les migrations manuelles sans snapshot sont silencieusement ignorees par Drizzle. Si une migration a deja ete appliquee a la main via sqlite3, inserer son hash SHA-256 dans `__drizzle_migrations` avec `shasum -a 256 fichier.sql`. **Apres chaque `drizzle-kit generate`**, verifier que les timestamps dans `drizzle/meta/_journal.json` sont strictement croissantes. Ne jamais supprimer un fichier de migration deja commite sans nettoyer aussi `__drizzle_migrations` et le journal. Ne jamais appliquer du SQL directement en dev sans enregistrer le hash — le migrator sautera la migration au prochain redemarrage.
